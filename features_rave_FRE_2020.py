...
def rave_timeseries(df, day_start_hour, sum_interval, use_weights):
    ...
    print(fire_rave_intersection_xr)
...
for jj in range(len(years)):
    ...
    for ii in range(1):#len(irwinIDs)):
        print(ii)
        fireID=irwinIDs[ii]
        print(fireID)
        ...
        print(rave)
        #rave.to_csv('./fire_features_fre/'+fireID+str(years[jj])+'_Daily_FRE_Weighted_'+str(start_time)+'Z_day_start.csv') #daily averages
