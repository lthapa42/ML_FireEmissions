...
start_time=12
resources_file  = '../YYYY_STRUCTURES.xlsx'
...
for jj in range(len(years)):
    ...
    resources_all = pd.read_excel(resources_file.replace('YYYY', str(years[jj]))).iloc[:,1:19]
    if years[jj]==2021:
        resources_all.columns = ['REPORT_FROM_DATE','REPORT_TO_DATE','IRWIN_IDENTIFIER','QTY_DESTROYED','QTY_DAMAGED','QTY_THREATENED_72']
    else:
        resources_all.columns = ['REPORT_FROM_DATE','REPORT_TO_DATE','PCT_CONTAINED_COMPLETED',
    ...
    for ii in range(len(irwinIDs)):
        ...
        resources = pd.concat([resources, pd.DataFrame({'irwinID':[fireID]*len(resources)})], axis=1)
        ...
        resources.to_csv('./fire_features_structures/'+fireID+str(years[jj])+'_Daily_STRUCTURES_'+str(start_time)+'Z_day_start.csv') #daily averages
